...
import {IContrat, IEnseignant} from "../../shared/model/models";
...
export class EnseignantService {
	...
	getAllDeactivated(): Observable<IEnseignant[]> {
		tap(enseignants => console.log("Enseignants: ", enseignants))
		catchError(this.handleError);
		return this.httpClient.get<IEnseignant[]>(this.API_URL + this.ENDPOINT_READ + '/deactivated');
	}

	getContracts(id: number): Observable<IContrat[]> {
		tap(maison => console.log("Maisons: ", maison))
		catchError(this.handleError);
		return this.httpClient.get<IContrat[]>(this.API_URL + this.ENDPOINT_READ + '/contract/' + id);
	}
